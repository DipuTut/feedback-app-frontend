pipeline {
    ...
    environment {
        GITHUB_REPO = 'https://github.com/DipuTut/feedback-app-frontend.git'        
        DOCKER_CREDENTIALS_ID = 'dockerhub-token'
        DOCKER_REPO = 'asadulhaque90/feedback-app-frontend'
    ...
    stages {        
        ...
        stage('Kubernetes Deploy API') {
            steps {
                ...
                container('kubectl') {
                    script {
                        sh 'sed -i "s|image: asadulhaque90/feedback-app-frontend:latest|image: $DOCKER_IMAGE|g" kubernetes/api-deployment.yaml'
